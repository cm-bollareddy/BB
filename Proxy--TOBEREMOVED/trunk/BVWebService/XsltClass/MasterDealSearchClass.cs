...
{
	/// <summary>
	/// MasterDealSearchClass: Implement the MasterDealSearch web method.
	/// </summary>
	class MasterDealSearchClass
	{
		private const string FILE_SEARCH_XSLT = @"MasterDealSearch.xslt";

		public MasterDealSearchClass()
		{
			//
		...
		public DataSet MasterDealSearch(string strSessionID, SearchCriterion[] searchCriteria, int nMaxRows)
		{
			...
			oTracer.LogInfo("Entering MasterDealSearch.\n searchParams=" + searchParams);
			...
			if (n == 0)
			{
				throw new ServiceException(
					...
					"No search field is found.  Search field supported is title",
					"",
				...
				if ((s != null) && (s.SearchField != null) && (s.SearchField != "") && (s.SearchValue != null) && (s.SearchValue != ""))
				{
					switch (s.SearchField)
					{
						...
						default:
						{
							throw new ServiceException(
								ServiceException.ExceptionCode.InvalidParameters,
								"Invalid search field: " + s.SearchField + ". Search field supported is title",
			...
			catch (Exception ex)
			{
				throw new BVException(
					...
					"Component= TVSServer.rdmPBSSearch MasterDealSearch \nCriteria(" + searchParams + ")",
					ex);
			}
			...
			try
			{
				nErrorCode = oPBSSearch.MasterDealSearch(strSessionID, oSearchParams, nMaxRows, out strSearchResult, out strErrorText);
			}
			catch (Exception ex)
			{
				throw new BVException(
