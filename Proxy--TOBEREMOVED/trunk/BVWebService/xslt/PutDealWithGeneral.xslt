	...
	<ROWPBSDEALCONTACT>
		...
		<xsl:choose>
			<xsl:when test="$RowState = $ROWSTATE_AFTER">
				<xsl:if test="(not(msxsl:node-set($Original)/PBSDC_ID) and PBSDC_ID) or (msxsl:node-set($Original)/PBSDC_ID != PBSDC_ID)">
					<xsl:attribute name="PBSDC_ID"><xsl:value-of select="PBSDC_ID" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/PBSDC_ID and not(PBSDC_ID)">
					<xsl:attribute name="PBSDC_ID"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/PBSDC_SLU_ID_CONTACTTYPE) and PBSDC_SLU_ID_CONTACTTYPE) or (msxsl:node-set($Original)/PBSDC_SLU_ID_CONTACTTYPE != PBSDC_SLU_ID_CONTACTTYPE)">
					<xsl:attribute name="PBSDC_SLU_ID_CONTACTTYPE"><xsl:value-of select="PBSDC_SLU_ID_CONTACTTYPE" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/PBSDC_SLU_ID_CONTACTTYPE and not(PBSDC_SLU_ID_CONTACTTYPE)">
					<xsl:attribute name="PBSDC_SLU_ID_CONTACTTYPE"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/PBSDC_CONTACTNAME) and PBSDC_CONTACTNAME) or (msxsl:node-set($Original)/PBSDC_CONTACTNAME != PBSDC_CONTACTNAME)">
					<xsl:attribute name="PBSDC_CONTACTNAME"><xsl:value-of select="PBSDC_CONTACTNAME" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/PBSDC_CONTACTNAME and not(PBSDC_CONTACTNAME)">
					<xsl:attribute name="PBSDC_CONTACTNAME"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/PBSDC_CONTACTPHONE) and PBSDC_CONTACTPHONE) or (msxsl:node-set($Original)/PBSDC_CONTACTPHONE != PBSDC_CONTACTPHONE)">
					<xsl:attribute name="PBSDC_CONTACTPHONE"><xsl:value-of select="PBSDC_CONTACTPHONE" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/PBSDC_CONTACTPHONE and not(PBSDC_CONTACTPHONE)">
					<xsl:attribute name="PBSDC_CONTACTPHONE"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/PBSDC_CONTACTEMAIL) and PBSDC_CONTACTEMAIL) or (msxsl:node-set($Original)/PBSDC_CONTACTEMAIL != PBSDC_CONTACTEMAIL)">
					<xsl:attribute name="PBSDC_CONTACTEMAIL"><xsl:value-of select="PBSDC_CONTACTEMAIL" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/PBSDC_CONTACTEMAIL and not(PBSDC_CONTACTEMAIL)">
					<xsl:attribute name="PBSDC_CONTACTEMAIL"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/PBSDC_COM_ID) and PBSDC_COM_ID) or (msxsl:node-set($Original)/PBSDC_COM_ID != PBSDC_COM_ID)">
					<xsl:attribute name="PBSDC_COM_ID"><xsl:value-of select="PBSDC_COM_ID" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/PBSDC_COM_ID and not(PBSDC_COM_ID)">
					<xsl:attribute name="PBSDC_COM_ID"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/PBSDC_DEA_ID) and PBSDC_DEA_ID) or (msxsl:node-set($Original)/PBSDC_DEA_ID != PBSDC_DEA_ID)">
					<xsl:attribute name="PBSDC_DEA_ID"><xsl:value-of select="PBSDC_DEA_ID" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/PBSDC_DEA_ID and not(PBSDC_DEA_ID)">
					<xsl:attribute name="PBSDC_DEA_ID"></xsl:attribute>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="PBSDC_ID"><xsl:value-of select="PBSDC_ID" /></xsl:attribute>
	...
	<ROW>
		...
		<PBSDEALCONTACT>
			<xsl:if test="($RowState=$ROWSTATE_NEW) or ($RowState=$ROWSTATE_BEFORE) or ($RowState=$ROWSTATE_UNCHANGED)">
				<xsl:for-each select="//DealWithGeneralDataSet/PBSDEALCONTACT">
					<xsl:choose>
						<xsl:when test="@diffgr:hasChanges='inserted'">
							<xsl:call-template name="OutputPBSDEALCONTACT">
								<xsl:with-param name="RowState" select="$ROWSTATE_NEW" />
							</xsl:call-template>
						</xsl:when>

						<xsl:when test="@diffgr:hasChanges='modified'">
							<xsl:for-each select="key('OriginalPBSDEALCONTACT', @diffgr:id)">
								<xsl:call-template name="OutputPBSDEALCONTACT">
									<xsl:with-param name="RowState" select="$ROWSTATE_BEFORE" />
								</xsl:call-template>
							</xsl:for-each>
							<xsl:call-template name="OutputPBSDEALCONTACT">
								<xsl:with-param name="Original" select="key('OriginalPBSDEALCONTACT', @diffgr:id)" />
								<xsl:with-param name="RowState" select="$ROWSTATE_AFTER" />
							</xsl:call-template>
						</xsl:when>
					</xsl:choose>
				</xsl:for-each>
				<xsl:for-each select="//diffgr:before/PBSDEALCONTACT">
					<xsl:if test="count(key('CurrentPBSDEALCONTACT', @diffgr:id)) = 0">
						<xsl:call-template name="OutputPBSDEALCONTACT">
...
<xsl:template match="/">
	...
	<ROWDATA>
		<xsl:for-each select="//DealWithGeneralDataSet/DEAL">
			<xsl:choose>
				<xsl:when test="@diffgr:hasChanges='inserted'">
