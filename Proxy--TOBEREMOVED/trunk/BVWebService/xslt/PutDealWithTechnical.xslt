	...
	<ROWDEALVERSIONTYPE>
		...
		<xsl:choose>
			<xsl:when test="$RowState = $ROWSTATE_AFTER">
				<xsl:if test="(not(msxsl:node-set($Original)/DVT_ID) and DVT_ID) or (msxsl:node-set($Original)/DVT_ID != DVT_ID)">
					<xsl:attribute name="DVT_ID"><xsl:value-of select="DVT_ID" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/DVT_ID and not(DVT_ID)">
					<xsl:attribute name="DVT_ID"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/DVT_DEA_ID) and DVT_DEA_ID) or (msxsl:node-set($Original)/DVT_DEA_ID != DVT_DEA_ID)">
					<xsl:attribute name="DVT_DEA_ID"><xsl:value-of select="DVT_DEA_ID" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/DVT_DEA_ID and not(DVT_DEA_ID)">
					<xsl:attribute name="DVT_DEA_ID"></xsl:attribute>
				</xsl:if>
				<xsl:if test="(not(msxsl:node-set($Original)/DVT_VET_ID) and DVT_VET_ID) or (msxsl:node-set($Original)/DVT_VET_ID != DVT_VET_ID)">
					<xsl:attribute name="DVT_VET_ID"><xsl:value-of select="DVT_VET_ID" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="msxsl:node-set($Original)/DVT_VET_ID and not(DVT_VET_ID)">
					<xsl:attribute name="DVT_VET_ID"></xsl:attribute>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="DVT_ID"><xsl:value-of select="DVT_ID" /></xsl:attribute>
	...
	<ROW>
		...
		<DEALVERSIONTYPE>
			<xsl:if test="($RowState=$ROWSTATE_NEW) or ($RowState=$ROWSTATE_BEFORE) or ($RowState=$ROWSTATE_UNCHANGED)">
				<xsl:for-each select="//DealWithTechnicalDataSet/DEALVERSIONTYPE">
					<xsl:choose>
						<xsl:when test="@diffgr:hasChanges='inserted'">
							<xsl:call-template name="OutputDEALVERSIONTYPE">
								<xsl:with-param name="RowState" select="$ROWSTATE_NEW" />
							</xsl:call-template>
						</xsl:when>

						<xsl:when test="@diffgr:hasChanges='modified'">
							<xsl:for-each select="key('OriginalDEALVERSIONTYPE', @diffgr:id)">
								<xsl:call-template name="OutputDEALVERSIONTYPE">
									<xsl:with-param name="RowState" select="$ROWSTATE_BEFORE" />
								</xsl:call-template>
							</xsl:for-each>
							<xsl:call-template name="OutputDEALVERSIONTYPE">
								<xsl:with-param name="Original" select="key('OriginalDEALVERSIONTYPE', @diffgr:id)" />
								<xsl:with-param name="RowState" select="$ROWSTATE_AFTER" />
							</xsl:call-template>
						</xsl:when>
					</xsl:choose>
				</xsl:for-each>
				<xsl:for-each select="//diffgr:before/DEALVERSIONTYPE">
					<xsl:if test="count(key('CurrentDEALVERSIONTYPE', @diffgr:id)) = 0">
						<xsl:call-template name="OutputDEALVERSIONTYPE">
...
<xsl:template match="/">
	...
	<ROWDATA>
		<xsl:for-each select="//DealWithTechnicalDataSet/DEAL">
			<xsl:choose>
				<xsl:when test="@diffgr:hasChanges='inserted'">
