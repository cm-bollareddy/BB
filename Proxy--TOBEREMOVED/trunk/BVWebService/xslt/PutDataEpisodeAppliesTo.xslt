...
<xsl:key name="OriginalEPISODEAPPTO" match="//diffgr:before/EPISODEAPPTO" use="@diffgr:id" />
<xsl:key name="CurrentEPISODEAPPTO" match="//EpisodeAppliesTo/EPISODEAPPTO" use="@diffgr:id" />

<xsl:template name="Output_Schema">
<!-- SCHEMA STARTS -->
<METADATA><FIELDS><FIELD attrname="ASS_ID" fieldtype="i4" required="true"><PARAM Name="PROVFLAGS" Value="7" Type="i4" Roundtrip="True" /><PARAM Name="ORIGIN" Value="ASSET.ASS_ID" Roundtrip="True" /></FIELD><FIELD attrname="ASS_EPISODENUMBER" fieldtype="string" WIDTH="20"><PARAM Name="ORIGIN" Value="ASSET.ASS_EPISODENUMBER" Roundtrip="True" /></FIELD><FIELD attrname="ASS_EPISODETITLE" fieldtype="string" WIDTH="80"><PARAM Name="ORIGIN" Value="ASSET.ASS_EPISODETITLE" Roundtrip="True" /></FIELD><FIELD attrname="TALENT_FOUND_FLAG" fieldtype="i4" readonly="true" /></FIELDS><PARAMS DATASET_DELTA="TRUE" PRIMARY_KEY="1" /></METADATA>
<!-- SCHEMA ENDS   -->
</xsl:template>

<xsl:include href="TransformUtil.xslt" />

<xsl:template name="OutputEPISODEAPPTO">
...
<xsl:template match="/">
	...
	<ROWDATA>
		<xsl:for-each select="//EpisodeAppliesTo/EPISODEAPPTO">
			<xsl:choose>
				<xsl:when test="@diffgr:hasChanges='inserted'">
					<xsl:call-template name="OutputEPISODEAPPTO">
						<xsl:with-param name="RowState" select="$ROWSTATE_NEW" />
					</xsl:call-template>
				</xsl:when>

				<xsl:when test="@diffgr:hasChanges='modified'">
					<xsl:for-each select="key('OriginalEPISODEAPPTO', @diffgr:id)">
						<xsl:call-template name="OutputEPISODEAPPTO">
							<xsl:with-param name="RowState" select="$ROWSTATE_BEFORE" />
						</xsl:call-template>
					</xsl:for-each>
					<xsl:call-template name="OutputEPISODEAPPTO">
						<xsl:with-param name="Original" select="key('OriginalEPISODEAPPTO', @diffgr:id)" />
						<xsl:with-param name="RowState" select="$ROWSTATE_AFTER" />
					</xsl:call-template>
				</xsl:when>
			</xsl:choose>
		</xsl:for-each>
		<xsl:for-each select="//diffgr:before/EPISODEAPPTO">
			<xsl:if test="count(key('CurrentEPISODEAPPTO', @diffgr:id)) = 0">
				<xsl:call-template name="OutputEPISODEAPPTO">
