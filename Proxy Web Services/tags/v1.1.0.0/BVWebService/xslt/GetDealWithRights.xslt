...
<xsl:template match="/">
	...
	<diffgr:diffgram xmlns:msdata="urn:schemas-microsoft-com:xml-msdata" xmlns:diffgr="urn:schemas-microsoft-com:xml-diffgram-v1">
		<DealWithRightsDataSet xmlns="http://localhost/BVWebService/xsd/DealWithRightsDataSet.xsd">
			<xsl:for-each select="DATAPACKET/ROWDATA/ROW">
				...
				<xsl:for-each select="DEALCONTENT/ROWDEALCONTENT">
					<DEALCONTENT>
					<xsl:attribute name="diffgr:id">DEALCONTENT<xsl:value-of select="position()" /></xsl:attribute>
					<xsl:attribute name="msdata:rowOrder"><xsl:value-of select="position() - 1" /></xsl:attribute>
						<DCO_ID><xsl:value-of select="@DCO_ID" /></DCO_ID>
						<xsl:if test="@DCO_CHA_ID != ''">
							<DCO_CHA_ID><xsl:value-of select="@DCO_CHA_ID" /></DCO_CHA_ID>
						</xsl:if>
						<xsl:if test="@DCO_TITLE != ''">
							<DCO_TITLE><xsl:value-of select="@DCO_TITLE" /></DCO_TITLE>
						</xsl:if>
						<xsl:if test="@DCO_ARR_ID != ''">
							<DCO_ARR_ID><xsl:value-of select="@DCO_ARR_ID" /></DCO_ARR_ID>
						</xsl:if>
						<xsl:if test="@DCO_DAYSAVAILABLE != ''">
							<DCO_DAYSAVAILABLE><xsl:value-of select="@DCO_DAYSAVAILABLE" /></DCO_DAYSAVAILABLE>
						</xsl:if>
						<xsl:if test="@DCO_DATEAVAILABLEFROM != ''">
							<DCO_DATEAVAILABLEFROM>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_DATEAVAILABLEFROM" /></xsl:with-param>
								</xsl:call-template>
							</DCO_DATEAVAILABLEFROM>
						</xsl:if>
						<xsl:if test="@DCO_DATEAVAILABLETO != ''">
							<DCO_DATEAVAILABLETO>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_DATEAVAILABLETO" /></xsl:with-param>
								</xsl:call-template>
							</DCO_DATEAVAILABLETO>
						</xsl:if>
						<xsl:if test="@DCO_ISOBBASED != ''">
							<DCO_ISOBBASED><xsl:value-of select="@DCO_ISOBBASED" /></DCO_ISOBBASED>
						</xsl:if>
						<xsl:if test="@DCO_ISEXCLUSIVE != ''">
							<DCO_ISEXCLUSIVE><xsl:value-of select="@DCO_ISEXCLUSIVE" /></DCO_ISEXCLUSIVE>
						</xsl:if>
						<xsl:if test="@DCO_BLACKOUTFROM != ''">
							<DCO_BLACKOUTFROM>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_BLACKOUTFROM" /></xsl:with-param>
								</xsl:call-template>
							</DCO_BLACKOUTFROM>
						</xsl:if>
						<xsl:if test="@DCO_BLACKOUTTO != ''">
							<DCO_BLACKOUTTO>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_BLACKOUTTO" /></xsl:with-param>
								</xsl:call-template>
							</DCO_BLACKOUTTO>
						</xsl:if>
						<xsl:if test="@DCO_BLACKOUTFROM2 != ''">
							<DCO_BLACKOUTFROM2>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_BLACKOUTFROM2" /></xsl:with-param>
								</xsl:call-template>
							</DCO_BLACKOUTFROM2>
						</xsl:if>
						<xsl:if test="@DCO_BLACKOUTTO2 != ''">
							<DCO_BLACKOUTTO2>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_BLACKOUTTO2" /></xsl:with-param>
								</xsl:call-template>
							</DCO_BLACKOUTTO2>
						</xsl:if>
						<xsl:if test="@DCO_BLACKOUTFROM3 != ''">
							<DCO_BLACKOUTFROM3>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_BLACKOUTFROM3" /></xsl:with-param>
								</xsl:call-template>
							</DCO_BLACKOUTFROM3>
						</xsl:if>
						<xsl:if test="@DCO_BLACKOUTTO3 != ''">
							<DCO_BLACKOUTTO3>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DCO_BLACKOUTTO3" /></xsl:with-param>
								</xsl:call-template>
							</DCO_BLACKOUTTO3>
						</xsl:if>
						<DCO_DEA_ID><xsl:value-of select="@DCO_DEA_ID" /></DCO_DEA_ID>
					</DEALCONTENT>
				</xsl:for-each>
			</xsl:for-each>
		</DealWithRightsDataSet>
	</diffgr:diffgram>
	</DataSet>
</xsl:template>
</xsl:stylesheet>
