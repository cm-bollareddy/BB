...
{
	/// <summary>
	/// AdCopySearchClass: Implement the AdCopySearch web method.
	/// </summary>
	class AdCopySearchClass
	{
		private const string FILE_SEARCH_XSLT = @"AdCopySearch.xslt";

		public AdCopySearchClass()
		{
			//
		...
		public DataSet AdCopySearch(string strSessionID, SearchCriterion[] searchCriteria, int nMaxRows)
		{
			...
			oTracer.LogInfo("Entering AdCopySearch.\n searchParams=" + searchParams);
			...
			if (n == 0)
			{
				throw new ServiceException(
					...
					"No search field is found.  Search field(s) supported are Title,Duration,Video_Format,Underwriter Id,Is_Approved,EXCLUDEINUSE.",
					"",
				...
				if ((s != null) && (s.SearchField != null) && (s.SearchField != "") && (s.SearchValue != null) && (s.SearchValue != ""))
				{
					switch (s.SearchField)
					{
						...
						default:
						{
							throw new ServiceException(
								ServiceException.ExceptionCode.InvalidParameters,
								"Invalid search field: " + s.SearchField + ". Search field(s) supported are Title,Duration,Video_Format,Underwriter Id,Is_Approved,EXCLUDEINUSE.",
			...
			catch (Exception ex)
			{
				throw new BVException(
					...
					"Component= TVSServer.rdmPBSSearch AdCopySearch \nCriteria(" + searchParams + ")",
					ex);
			}
			...
			try
			{
				nErrorCode = oPBSSearch.AdCopySearch(strSessionID, oSearchParams, nMaxRows, out strSearchResult, out strErrorText);
			}
			catch (Exception ex)
			{
				throw new BVException(
