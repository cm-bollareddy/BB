...
<xsl:template match="/">
	...
	<diffgr:diffgram xmlns:msdata="urn:schemas-microsoft-com:xml-msdata" xmlns:diffgr="urn:schemas-microsoft-com:xml-diffgram-v1">
		<DealWithFundingDataSet xmlns="http://localhost/BVWebService/xsd/BVWebService/DealWithFundingDataSet.xsd">
			<xsl:for-each select="DATAPACKET/ROWDATA/ROW">
				...
				<xsl:for-each select="MASTERDEAL/ROWMASTERDEAL">
					<MASTERDEAL>
					...
					<xsl:attribute name="msdata:rowOrder"><xsl:value-of select="position() - 1" /></xsl:attribute>
						...
						<xsl:if test="@MDL_PRODUCTIONCOST != ''">
							<MDL_PRODUCTIONCOST><xsl:value-of select="@MDL_PRODUCTIONCOST" /></MDL_PRODUCTIONCOST>
						</xsl:if>
					</MASTERDEAL>
				</xsl:for-each>
				<xsl:for-each select="MASTERDEALFUNDING/ROWMASTERDEALFUNDING">
					<MASTERDEALFUNDING>
					<xsl:attribute name="diffgr:id">MASTERDEALFUNDING<xsl:value-of select="position()" /></xsl:attribute>
					<xsl:attribute name="msdata:rowOrder"><xsl:value-of select="position() - 1" /></xsl:attribute>
						<MDF_ID><xsl:value-of select="@MDF_ID" /></MDF_ID>
						<MDF_MDL_ID><xsl:value-of select="@MDF_MDL_ID" /></MDF_MDL_ID>
						<MDF_COM_ID_FUNDER><xsl:value-of select="@MDF_COM_ID_FUNDER" /></MDF_COM_ID_FUNDER>
						<xsl:if test="@MDF_SLU_ID_FUNDINGTYPE != ''">
							<MDF_SLU_ID_FUNDINGTYPE><xsl:value-of select="@MDF_SLU_ID_FUNDINGTYPE" /></MDF_SLU_ID_FUNDINGTYPE>
						</xsl:if>
						<xsl:if test="@MDF_AMOUNT != ''">
							<MDF_AMOUNT><xsl:value-of select="@MDF_AMOUNT" /></MDF_AMOUNT>
						</xsl:if>
						<xsl:if test="@MDF_ISCREDITED != ''">
							<MDF_ISCREDITED><xsl:value-of select="@MDF_ISCREDITED" /></MDF_ISCREDITED>
						</xsl:if>
						<xsl:if test="@MDF_BEGINDATE != ''">
							<MDF_BEGINDATE>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@MDF_BEGINDATE" /></xsl:with-param>
								</xsl:call-template>
							</MDF_BEGINDATE>
						</xsl:if>
						<xsl:if test="@MDF_ENDDATE != ''">
							<MDF_ENDDATE>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@MDF_ENDDATE" /></xsl:with-param>
								</xsl:call-template>
							</MDF_ENDDATE>
						</xsl:if>
						<xsl:if test="@MDF_FISCALYEAR != ''">
							<MDF_FISCALYEAR><xsl:value-of select="@MDF_FISCALYEAR" /></MDF_FISCALYEAR>
						</xsl:if>
						<xsl:if test="@MDF_ISPRODUCERFUNDING != ''">
							<MDF_ISPRODUCERFUNDING><xsl:value-of select="@MDF_ISPRODUCERFUNDING" /></MDF_ISPRODUCERFUNDING>
						</xsl:if>
					</MASTERDEALFUNDING>
				</xsl:for-each>
				<xsl:for-each select="DEALFUNDING/ROWDEALFUNDING">
					<DEALFUNDING>
					<xsl:attribute name="diffgr:id">DEALFUNDING<xsl:value-of select="position()" /></xsl:attribute>
					<xsl:attribute name="msdata:rowOrder"><xsl:value-of select="position() - 1" /></xsl:attribute>
						<DEF_ID><xsl:value-of select="@DEF_ID" /></DEF_ID>
						<DEF_DEA_ID><xsl:value-of select="@DEF_DEA_ID" /></DEF_DEA_ID>
						<DEF_COM_ID_FUNDER><xsl:value-of select="@DEF_COM_ID_FUNDER" /></DEF_COM_ID_FUNDER>
						<xsl:if test="@DEF_SLU_ID_FUNDINGTYPE != ''">
							<DEF_SLU_ID_FUNDINGTYPE><xsl:value-of select="@DEF_SLU_ID_FUNDINGTYPE" /></DEF_SLU_ID_FUNDINGTYPE>
						</xsl:if>
						<xsl:if test="@DEF_AMOUNT != ''">
							<DEF_AMOUNT><xsl:value-of select="@DEF_AMOUNT" /></DEF_AMOUNT>
						</xsl:if>
						<xsl:if test="@DEF_FISCALYEAR != ''">
							<DEF_FISCALYEAR><xsl:value-of select="@DEF_FISCALYEAR" /></DEF_FISCALYEAR>
						</xsl:if>
						<xsl:if test="@DEF_ISCREDITED != ''">
							<DEF_ISCREDITED><xsl:value-of select="@DEF_ISCREDITED" /></DEF_ISCREDITED>
						</xsl:if>
						<xsl:if test="@DEF_BEGINDATE != ''">
							<DEF_BEGINDATE>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DEF_BEGINDATE" /></xsl:with-param>
								</xsl:call-template>
							</DEF_BEGINDATE>
						</xsl:if>
						<xsl:if test="@DEF_ENDDATE != ''">
							<DEF_ENDDATE>
								<xsl:call-template name="DateConverter">
									<xsl:with-param name="InDate"><xsl:value-of select="@DEF_ENDDATE" /></xsl:with-param>
								</xsl:call-template>
							</DEF_ENDDATE>
						</xsl:if>
						<xsl:if test="@DEF_ISPRODUCERFUNDING != ''">
							<DEF_ISPRODUCERFUNDING><xsl:value-of select="@DEF_ISPRODUCERFUNDING" /></DEF_ISPRODUCERFUNDING>
						</xsl:if>
					</DEALFUNDING>
				</xsl:for-each>
			</xsl:for-each>
		</DealWithFundingDataSet>
	</diffgr:diffgram>
	</DataSet>
</xsl:template>
</xsl:stylesheet>
